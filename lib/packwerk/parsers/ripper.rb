    ...
    class Ripper
      ...
      class RipperToast < ::Ripper
        ...
        def on_string_embexpr(stmts)
          stmts.is_a?(Array) ? s(:begin, *stmts) : stmts
        end

        def on_const_path_ref(left, const)
          set_const_parent(const, left)
        end
        ...
        def on_top_const_ref(const)
          set_const_parent(const, s(:cbase))
        end
        ...
        def on_def(ident, params, stmts)
          s(:def, ident, params, *stmts)
        end

        def on_yield(args)
          s(:yield, sequence(args))
        end
        ...
        def on_label(name)
          s(:sym, name[0..-2].to_sym)
        end

        def on_tlambda(_value)
          nil
        end

        def on_tlambeg(_value)
          nil
        end

        def on_magic_comment(_key, _value)
          nil
        end
        ...
        unhandled_events.each do |event|
          name = "on_#{event}"
          define_method(name.to_sym) { |*params| raise NotImplementedError, "#{name} (params #{params.inspect})" }
        end
      end
    end
  end
