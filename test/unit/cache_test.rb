  ...
  class CacheTest < Minitest::Test
    ...
    test "#todo writes to the cache" do
      ...
      unresolved_references = [
        Private::UnresolvedReference.new(
          constant_name: "MyConstant",
          namespace_path: [],
          relative_path: "path/of_exile.rb",
          source_location: Private::Node::Location.new(5, 5)
        ),
      ]

      Private::FileProcessor.any_instance.stubs(:references_from_ast).returns(unresolved_references)
      configuration = Configuration.from_path
      configuration.stubs(cache_enabled?: true)

      parse_run = Private::ParseRun.new(relative_file_set: Set.new([filepath.to_s]), configuration: configuration)
      ...
      cached_result = Private::Cache::CacheContents.deserialize(digest_file.read)
